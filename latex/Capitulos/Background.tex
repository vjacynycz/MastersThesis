...
Many of the journals with high impact factors belong to \bb{publishers}. In the
...
interlinked data called \ii{blockchain} (see figure~\ref{tb:bc:transaction}). To
...
\input{Capitulos/listing_contract.tex}
...
execution has a fee called \bb{gas} that users have to pay to the network based
on how complex is the code they want to run. The \bb{gas} amount required to
execute a function inside a smart contract directly determines the code quality,
as expensive transactions to smart contracts with high \bb{gas} costs are
undesirable.
...
\bb{Event Management Systems} (EMS) or \ii{Conference Management Systems} are
wide range of software products that are used in the management academic
conferences~\cite{azar2006academic}.

\bb{EasyChair}~\cite{easychair} is perhaps one of the best known, a web-based
...
\bb{OpenConf}~\cite{openconf} is another EMS that offers the same tools as
...
\bb{AEMS}~\cite{paulo2011aems} is an advanced event management system that
...
the monitoring of this process there are several platforms called \bb{Editorial
  systems} (ES)~\cite{Lev2016}.

\bb{Evise} is a web-based ES used by the publisher ``Elsevier''~\cite{evise} to
...
\bb{MySpringer}~\cite{myspringer} by ``Springer Nature'' is another ES used to
...
\bb{Publons}~\cite{rajpert2016rewarding}, a platform in which users make public
...
most promising ones is \bb{Aletheia}, a peer review proposal that tries to solve
...
of \ii{open acess}, as the information held by this system is public and free.
It also embraces ideas like \ii{preprints}, as paper submissions are accessible
before the peer review process, and \ii{continuous publication}, as every time a
